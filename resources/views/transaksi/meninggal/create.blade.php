            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-5">
                ...
                <form action="{{route('meninggal.store')}}" method="post">
                    ...
                    <fieldset class="border-solid border-blue-500 border-2 px-4 pb-4 mt-5">
                        ...
                        <input type="text" name="tempat" value="{{old('tempat')}}" placeholder="Bandung ..." class="rounded-md px-4 py-2  focus:outline-none bg-gray-100 lg:w-1/2 sm:w-full"/>
                        @error('tempat')
                            <div class="text-red-500">{{ $message }}</div>
                        @enderror

                        <label for="dimakamkan_di" class="block text-black mt-3 font-bold">Dimakamkan di:</label>
                        <input type="text" name="dimakamkan_di" value="{{old('dimakamkan_di')}}" placeholder="TPU ..." class="rounded-md px-4 py-2  focus:outline-none bg-gray-100 lg:w-1/2 sm:w-full"/>
                        @error('dimakamkan_di')
                            <div class="text-red-500">{{ $message }}</div>
                        @enderror
