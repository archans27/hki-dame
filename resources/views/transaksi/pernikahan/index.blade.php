    ...
    <div class="py-5" style="clear: both">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8" >
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
              ...
              <form action="{{route('pernikahan.create')}}" method="get" class="float-right m-5">
                    ...
                    <table class="min-w-full table-auto">
                        <thead class="justify-between">
                          <tr class="bg-gray-800 text-white py-5">
                            <th class="p-2 text-left">Kepala Keluaraga</th>
                            <th class="p-2">Mempelai</th>
                            <th class="p-2">Pasangan</th>
                            <th class="p-2">Tanggal Pernikahan</th>
                        ...
                        <tbody class="bg-gray-200">
                            @foreach ($pernikahans as $pernikahan)
                              <tr class="bg-white border-4 border-gray-200 items-center text-gray-700 hover:bg-gray-200">
                                <td class="p-2 flex flex-row text-center cursor-pointer font-bold text-blue-500 hover:text-yellow-500" ><a href="{{url('/pernikahan/'.$pernikahan->id)}}">{{$pernikahan->kepala_keluarga}}</a></td>
                                ...
                                <td class="p-2 text-left align-middle" >
                                  <form action="{{url('pernikahan/'.$pernikahan->id.'/edit')}}" class="float-left">
                                    <button type="submit">
                                  ...
                                  <form method="post" action="{{url('/pernikahan/'.$pernikahan->id)}}" class="float-left">
                                    @method('DELETE')
                                    @csrf
                                    <button type="submit"
                                      onclick="event.preventDefault();
                                        toggleModal('modal-delete');
                                        deletePernikahan('{{url('/pernikahan/'.$pernikahan->id)}}')"
                                    >
                                      <span class="material-icons cursor-pointer text-gray-500 hover:text-blue-500">
                                        delete
                                        </span>
