...
class DashboardController extends Controller
{
    ...
    public function index()
    {
        ...
        $keluarga = DB::table('keluarga')
            ...
            $sektorData = DB::table('sektor')
            ->select('sektor.nama as nama_sektor', DB::raw('count(keluarga.id) as jumlah_kk'))
            ->leftJoin('keluarga', 'sektor.id', '=', 'keluarga.sektor_id')
            ->groupBy('sektor.nama')
            ->get();


        $sintuaData = DB::table('sintua')
            ->select('sektor.nama as nama_sektor', DB::raw('count(*) as jumlah_sintua'))
        ...
        return view('dashboard', [
            ...
            'count_jk' => $count_jk,
            'count_golda' => $count_golda,
            'sektorData' => $sektorData,
            'sintuaData' => $sintuaData,
        ]);
    }
}
